...
export default function VendorPackages() {
  ...
  const renderStatusTabs = () => (
    <View style={styles.tabContainer}>
      {(["all", "pending", "approved", "rejected"] as PackageStatus[]).map(
        (status) => (
          <TouchableOpacity
            key={status}
            style={[
  ...
  const renderPackageItem = (item: VendorPackage) => (
    <TouchableOpacity
      key={item.id}
      style={styles.card}
      onPress={() => router.push(`/package-vendor/${item.id}` as any)}
    >
      {/* Header Card: Tên + Status */}
      <View style={styles.cardHeader}>
        <View style={{ flex: 1 }}>
          <Text style={styles.cardCategory}>
            {item.category?.name || "Dịch vụ"}
          </Text>
          ...
          <Pressable style={styles.modalContent}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TextInput
                ...
                onChangeText={(v) => setFormData({ ...formData, features: v })}
              />
